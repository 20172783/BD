...
public class ShedulePanel : MonoBehaviour
{
    ...
    public GameObject SheduleDayPrefab;
    public Transform SheduleDayContent;

    public GameObject SheduleHourPrefab;
    public Transform SheduleHourContent;
    ...
    {
        GameObject.Find("FirebaseManager").GetComponent<FirebaseManager>().GetWorkerShedule(this.gameObject);
    }
    public void spawnDays()
    {
        foreach (Transform child in SheduleDayContent.transform)
        {
        ...
        foreach (DateTime day in EachDay(StartDate, EndDate))
        {
            ...
            if (!busy)
            {
                GameObject OrderDayListElement = Instantiate(SheduleDayPrefab, SheduleDayContent);
                OrderDayListElement.GetComponent<SheduleDayElement>().NewElement(day.DayOfWeek.ToString(), day.Month, day.Day, day);
            }
        }
    }
    ...
    {
        foreach (Transform child in SheduleHourContent.transform)
        {
            Destroy(child.gameObject);
        }
        ...
        {
            GameObject OrderHourListElement = Instantiate(SheduleHourPrefab, SheduleHourContent);
        }
    }
